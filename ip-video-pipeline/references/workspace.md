...
开工前检查并安装缺失依赖，建立共享素材库和每条视频的独立工作区，生成项目规则、素材索引及路径清单。后续制作步骤根据配置定位文件。

## 1. 先检查，再安装缺项

先执行 `pipeline.py doctor`，读取当前系统和已安装工具，只安装所选制作路线缺少的依赖，不因新建视频重复安装。

- 必需：FFmpeg/FFprobe、Python。音频模型环境建议Python3.11/3.12；按当前模型依赖选择兼容版本。
- 当前HyperFrames路线必需：Node.js22或更新版、HyperFrames及对应Skill、生图工具。
